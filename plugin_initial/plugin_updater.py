...
class SimpleHandler(Handler):
    ...
    def handle_connect(self):
        ...
        print('Keeping listening if new networkid added...')
    ...
    def handle_notification(self, notification):
        if notification.notification == '$device-add' or notification.notification == '$device-update':
            ...
            if networkid not in networkIds:
                ...
                self.inactive_plugin(self.influxPluginTopicName)
                plugin_headers = {'Authorization': 'Bearer ' + self.get_access_token()}
                url = f"{self.plugin_url}?topicName={self.influxPluginTopicName}&{result_str}&returnCommands=true&returnUpdatedCommands=true&returnNotifications=true&status=ACTIVE"
...
def run_updater():
    ...
    Manager = Plugin(SimpleHandler, config_json)
    Manager.connect(proxy_endpoint = config_json['proxyEndpoint'], 
                   topic_name = config_json['listenerPluginTopicName'], 
